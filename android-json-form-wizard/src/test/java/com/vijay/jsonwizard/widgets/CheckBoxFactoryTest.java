...
public class CheckBoxFactoryTest extends BaseTest {
    ...
    public void setUp() {
        ...
        context = Robolectric.buildActivity(JsonFormActivity.class, getJsonFormActivityIntent()).create().get();
    ...
    public void testTimePickerFactoryInstantiatesViewsCorrectly() throws Exception {
        ...
        JsonFormActivity jsonFormActivity = Mockito.spy(context);
...
//        rootLayout.setTag(R.id.address, "RandomStepName:delivery_complications");
        Mockito.doReturn(rootLayout).when(factorySpy).getLinearLayout(jsonFormActivity);
        Mockito.doReturn("RandomStepName:delivery_complications").when(rootLayout).getTag(R.id.address);
        Mockito.doReturn(checkboxLayout).when(factorySpy).getCheckboxLayout(jsonFormActivity);
        Mockito.doReturn(displayMetrics).when(resources).getDisplayMetrics();
        Mockito.doReturn(constraintLayout).when(formUtilsSpy).getRootConstraintLayout(jsonFormActivity);
        ...
        Mockito.doReturn(constraintLayoutTwo).when(formUtilsSpy).getConstraintLayout(ArgumentMatchers.anyString(), ArgumentMatchers.eq(new JSONArray()), ArgumentMatchers.eq(checkBoxWidget), ArgumentMatchers.eq(jsonFormActivity), ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), ArgumentMatchers.anyString());
        ...
        Mockito.doReturn(labelConstraintLayout).when(formUtilsSpy).createLabelLinearLayout(ArgumentMatchers.anyString(), ArgumentMatchers.eq(new JSONArray()), ArgumentMatchers.eq(new JSONObject()), ArgumentMatchers.eq(jsonFormActivity), ArgumentMatchers.eq(listener));

        //noinspection ResultOfMethodCallIgnored
        Mockito.doReturn(jsonFormActivity).when(formFragment).getJsonApi();

        List<View> viewList = factorySpy.getViewsFromJson("RandomStepName", jsonFormActivity, formFragment, checkBoxWidget, listener);
